...
class Graph_Rec(torch.nn.Module):
    def __init__(self, input_size, hidden_size1, hidden_size2,
                 output_size, n_layer, n_user, n_item, n_attr, n_relation, embed_size, mapping):
        ...
        # 训练一下TransDmoxing
        self.kg_model = TransD(ent_tot=n_user + n_item + n_attr + 1, rel_tot=cfg.KG_RELA_COUNT+1,
        ...
        self.user_user_rel = load_pickle("./pickle/user_user_rel.pkl")
        self.user_item_rel = load_pickle("./pickle/user_item_rel.pkl")
        self.item_attr_rel = load_pickle("./pickle/item_attr_rel.pkl")
    ...
    def update_weight(self):
        ...
        all_rel = torch.tensor([iii for iii in range(self.n_relation + 1)], device=cfg.device,dtype=torch.float32)

        all_id_embedding = self.kg_model.ent_embedding(all_id).detach()
        all_id_transfer = self.kg_model.ent_embedding(all_id).detach()
        all_rel_embedding = self.kg_model.rel_embedding(all_rel).detach()
        all_rel_transfer = self.kg_model.rel_embedding(all_rel).detach()
        self.sp_weight = dict()
        length = 1000
        current_id = 0
        n_fold = 0
